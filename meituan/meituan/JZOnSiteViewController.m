...
-(void)getHomeServiewData{
    ...
    [[NetworkSingleton shareManager] getHomeServiceResult:nil url:urlStr successBlock:^(id responseBody){
        NSLog(@"上门服务请求成功");
...
-(void)getServiceAdvData{
    ...
    [[NetworkSingleton shareManager] getServiceAdvResult:nil url:urlStr successBlock:^(id responseBody){
        NSLog(@"上门服务广告请求成功");
...
-(CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath{
    if (indexPath.row == 0) {
        ...
        if (_homeServiceArray.count!=0) {
            NSUInteger y = (_homeServiceArray.count +2-1)/2;
            return 125*y+5;
        }else{
            return 125*5+5;
        }
