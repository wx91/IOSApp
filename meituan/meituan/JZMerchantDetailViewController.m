...
-(void)getMerchantDetailData{
    ...
    [[NetworkSingleton shareManager] getMerchantDetailResult:nil url:urlStr successBlock:^(id responseBody){
        NSLog(@"商家详情请求成功");
...
-(void)getAroundGroupPurchaseData{
    ...
    NSString *urlStr = [NSString stringWithFormat:@"%@%@%@",str1,self.poiid,str2];
    [[NetworkSingleton shareManager] getAroundGroupPurchaseResult:nil url:urlStr successBlock:^(id responseBody){
        NSLog(@"附近团购请求成功");
