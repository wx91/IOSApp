...
-(void)initData{
    ...
    //"emoticons.plist"是个数组
    NSString *filePath=[[NSBundle mainBundle]pathForResource:@"emoticons"ofType:@"plist"];
    //目前这个fileArray有没有分类的104个表情，现在我们需要把它分类成上面注释中items的形式
...
-(void)drawRect:(CGRect)rect{
    ...
    for (int i=0; i<items.count; i++) {
        ...
        for (int j=0; j<items2D.count; j++) {
            ...
            CGRect emotionFrame=CGRectMake(column*item_width+15, row*item_height+15, 30, 30);
            //考虑页数，需要加上前面一页的高度
...
-(void)touchEmotion:(CGPoint)point{
    int page=point.x/ScreenWidth;//页面
    ...
    if (!(page == 3 && index > 19)) {
        ...
        if (![self.selectedEmotion isEqualToString:emotionName]||self.selectedEmotion==nil ) {
            ...
            UIImageView *bigFace=(UIImageView *)[magnifierView viewWithTag:111111];
